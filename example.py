...
class ExampleProblem:
    ...
    def true_distribution(self, x):
        ...
        return peak1_integral * stats.cauchy.pdf(x, -3, 0.5) + peak2_integral * stats.norm.pdf(x, 5, 1)
        #return peak1_integral * stats.norm.pdf(x, -1, 0.5) + peak2_integral * stats.norm.pdf(x, 2, 1)
    ...
    def test_distribution(self, x):
        ...
        print(peak1_integral, peak2_integral)
        return peak1_integral * stats.norm.pdf(x, -3.5, 1) + peak2_integral * stats.cauchy.pdf(x, 3.5, 0.5)
        #return peak1_integral * stats.norm.pdf(x, -1.5, 1) + peak2_integral * stats.norm.pdf(x, 1.5, 0.5)

    def __generate_initial_MC_peak1(self):
        self.__x_ini_1 = stats.cauchy.rvs(-3, 0.5, 1_000)
        #self.__x_ini_1 = np.random.normal(-1, 0.5, 10_000)

    def __generate_initial_MC_peak2(self):
        self.__x_ini_2 = np.random.normal(5, 1, 5_000)
    ...
    def __generate_test_distribution_peak1(self):
        self.__x_test_1 = np.random.normal(-3.5, 1, 3_000)

    def __generate_test_distribution_peak2(self):
        self.__x_test_2 = stats.cauchy.rvs(3.5, 0.5, 3_000)
        #self.__x_test_2 = np.random.normal(1.5, 0.5, 50_000)
